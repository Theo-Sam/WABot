...
const commands = [
  {
    ...
    handler: async (sock, m, { text }) => {
      ...
      try {
        ...
        const url = `https://tenor.googleapis.com/v2/featured?key=AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v6&q=${encodeURIComponent(emoji1)}_${encodeURIComponent(emoji2)}`;
        const data = await fetchJson(url);
        if (!data?.results?.length) return m.reply("❌ No emoji mix found for this combination.");
  ...
  },
  {
    ...
    handler: async (sock, m, { text }) => {
      ...
      m.react("⏳");
      try {
        const url = `https://tenor.googleapis.com/v2/search?key=AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ&q=${encodeURIComponent(text)}&limit=1&media_filter=gif`;
        const data = await fetchJson(url);
        if (!data?.results?.length) return m.reply("❌ No GIFs found.");
