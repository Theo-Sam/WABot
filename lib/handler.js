...
async function handleMessage(sock, rawMsg) {
  ...
  if (isCommand) {
    ...
    // Dot-emoji shortcut: .🔥 .❤️ .🎉 etc. while replying to a status → treat as .save
    // Works from both the status tab (remoteJid = status@broadcast set by connection.js)
    // and from the poster's DM (remoteJid = status@broadcast set by serialize.js contextInfo fix).
    let effectiveCmdName = cmdName;
    if (
      /^\P{ASCII}/u.test(cmdName) &&
      m.quoted?.key?.remoteJid === "status@broadcast"
    ) {
      effectiveCmdName = "save";
      console.log(`[DESAM-HANDLER] Dot-emoji "${cmdName}" in status context → remapped to "save"`);
    }

    const cmd = commands.get(effectiveCmdName);
    if (cmd) {
      commandMatched = true;
  ...
  if (!commandMatched && isGroup && m.body && !m.fromMe && groupSettings && groupSettings.chatbot && helpersModule) {
    ...
    try {
      ...
      if (question.length > 0 && question.length < 1000) {
        ...
        if (answer) {
          await m.reply(normalizeAiText(answer, { keepLightFormatting: true }));
        }
      }
    } catch { }
  }
