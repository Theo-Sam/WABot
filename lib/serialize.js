...
function serialize(sock, msg) {
  ...
  m.sender = m.isGroup
    ? jidNormalizedUser(msg.key.participant || m.chat)
    : m.fromMe
  ...
  if (quoted) {
    ...
    m.quoted = {
      key: {
        // Use contextInfo.remoteJid when WhatsApp provides it (e.g. status@broadcast for status
        // replies routed to a private DM). Fall back to m.chat for normal quoted messages.
        remoteJid: content.contextInfo.remoteJid || m.chat,
