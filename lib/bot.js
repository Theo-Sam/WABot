    ...
    if (connection === 'open') {
      ...
      try {
        ...
        if (notifyJid && notifyJid !== '@s.whatsapp.net') {
          ...
          const channelCtx = { externalAdReply: { title: config.BOT_NAME, body: "View Channel", mediaType: 1, renderLargerThumbnail: false, showAdAttribution: false, sourceUrl: "https://whatsapp.com/channel/0029Vb7n5HyEgGfKW3Wp7U1h" } };
          const imgPath = require('path').join(__dirname, '..', 'public', 'desam-bot.png');
          let imgBuffer = null;
          try { imgBuffer = require('fs').readFileSync(imgPath); } catch {}
          if (imgBuffer) {
            await sock.sendMessage(notifyJid, { image: imgBuffer, caption, contextInfo: channelCtx });
          } else {
            await sock.sendMessage(notifyJid, { text: caption, contextInfo: channelCtx });
          }
