    ...
    if (sock.ev && sock.ev.on) {
      ...
      const processAntiDelete = async (key) => {
        try {
          ...
          // Where to send the recovered content:
          //   • groups  → post in the group itself (everyone sees it)
          //   • DMs     → send to owner's saved messages
          const notifyDest = isGroup ? chat : (ownerJid || chat);

          const stored = await store.getMessage(key);
          logger.info(`[ANTI-DEL] store lookup for ${key.id}: ${stored ? 'FOUND' : 'NOT FOUND'}`);

          if (!stored?.message) {
            // Message not cached — still inform in the chat so people know
            await sock.sendMessage(notifyDest, {
              text: `🗑️ *Deleted message detected*\n${chatIcon} ${chatLabel}\n_Content not cached — delete happened before bot saw the message._`,
            }).catch(() => {});
            return;
          }
