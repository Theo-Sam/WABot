        ...
        try {
          ...
          for (const msg of messages) {
            ...
            if (isStatusLikeJid(remoteJid) || isStatusLikeJid(msg.key?.participant || '')) {
              ...
              if (msg.key?.fromMe || msg.key?.participant === sock.user?.id?.replace(/:.*@/, '@') + '@s.whatsapp.net') {
                const unwrapped = unwrapMessage(msg.message);
                const msgType = getContentType(unwrapped) || Object.keys(unwrapped || {})[0];
      ...
      sock.ev.on('messages.delete', async (item) => {
        try {
          ...
          for (const key of item.keys) {
            ...
            const body = normalizedMessage?.conversation || content?.text || content?.caption || '';
            ...
            if (['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(type)) {
              ...
              if (mediaBuffer && mediaBuffer.length > 0) {
                if (type === 'imageMessage') {
                  await sock.sendMessage(ownerJid, { image: mediaBuffer, caption: `${header}${body ? `\n📝 ${body}` : ''}`, mentions: senderJid ? [senderJid] : [] }).catch(() => {});
                } else if (type === 'videoMessage') {
                  await sock.sendMessage(ownerJid, { video: mediaBuffer, caption: `${header}${body ? `\n📝 ${body}` : ''}`, mentions: senderJid ? [senderJid] : [] }).catch(() => {});
                } else if (type === 'audioMessage') {
                  await sock.sendMessage(ownerJid, { audio: mediaBuffer, mimetype: content?.mimetype || 'audio/mpeg', ptt: false }).catch(() => {});
                  await sock.sendMessage(ownerJid, { text: `${header}${body ? `\n📝 ${body}` : ''}`, mentions: senderJid ? [senderJid] : [] }).catch(() => {});
                } else if (type === 'documentMessage') {
                  await sock.sendMessage(ownerJid, { document: mediaBuffer, fileName: content?.fileName || 'deleted-file', mimetype: content?.mimetype || 'application/octet-stream', caption: `${header}${body ? `\n📝 ${body}` : ''}`, mentions: senderJid ? [senderJid] : [] }).catch(() => {});
                } else if (type === 'stickerMessage') {
                  await sock.sendMessage(ownerJid, { sticker: mediaBuffer }).catch(() => {});
                  await sock.sendMessage(ownerJid, { text: header, mentions: senderJid ? [senderJid] : [] }).catch(() => {});
                }
                continue;
              }
            }
