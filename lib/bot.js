    ...
    if (sock.ev && sock.ev.on) {
      ...
      sock.ev.on('messages.update', async (updates) => {
        ...
        for (const update of updates) {
          ...
          // Check for REVOKE protocolMessage arriving via messages.update
          const proto = update?.message?.protocolMessage;
          if (proto) {
            const isKnownNonDelete = proto.type !== null && proto.type !== undefined && proto.type !== 0;
            if (!isKnownNonDelete && proto.key?.id) {
              const revokedKey = proto.key;
              if (!revokedKey.remoteJid) revokedKey.remoteJid = update.key?.remoteJid;
              if (!revokedKey.participant && update.key?.participant) revokedKey.participant = update.key.participant;
      ...
      // ── Backup trigger: protocolMessage REVOKE in messages.upsert ──
      ...
      // NOTE: proto.type is 0 (REVOKE) in most cases, but some WA/Baileys versions
      // send it as null or omit it — so we detect a REVOKE by checking proto.key.id.
      sock.ev.on('messages.upsert', async ({ messages: upsertMsgs }) => {
        if (!Array.isArray(upsertMsgs)) return;
        for (const msg of upsertMsgs) {
          const proto = msg?.message?.protocolMessage;
          if (!proto) continue;
          // Accept type 0 (REVOKE), null, or undefined — as long as proto.key.id is present
          // and it's not one of the clearly non-delete types (app sync, call log, etc.)
          const isKnownNonDelete = proto.type !== null && proto.type !== undefined && proto.type !== 0;
          if (isKnownNonDelete) continue;
          const revokedKey = proto.key;
          if (!revokedKey?.id) continue;
          // Fill in remoteJid/participant from the outer message if the proto key is missing them
          if (!revokedKey.remoteJid) revokedKey.remoteJid = msg.key?.remoteJid;
          if (!revokedKey.participant && msg.key?.participant) revokedKey.participant = msg.key.participant;
          await processAntiDelete(revokedKey);
        }
      });
