...
async function startBot() {
  try {
    ...
    if (!hasIncomingSession) {
      logger.error('❌ SESSION_ID is missing. This bot is configured to use SESSION_ID/.env as the only auth source.');
      logger.error('   Set SESSION_ID (or SESSION_FILE) in .env and restart.');
      process.exit(1);
    }
    ...
    // Keep auth_state synchronized with SESSION_ID from .env
    ...
    // This build requires SESSION_ID-driven auth for consistent cross-platform deployments.
    const needsPairing = !state.creds.registered;
    if (needsPairing) {
      logger.error('❌ No valid registered auth state available.');
      logger.error('   Provide a valid SESSION_ID in .env (or SESSION_FILE) and restart.');
      process.exit(1);
    }
  ...
  try {
    ...
    if (connection === 'close') {
      ...
      if (isConflict) {
        ...
        logger.error('⛔ Session is invalid or logged out. Update SESSION_ID in .env and restart.');
        process.exit(1);
      }
    }
