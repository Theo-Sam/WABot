...
async function startBot() {
  try {
    ...
    // Only import SESSION_ID on first run (no existing creds).
    // On subsequent restarts, keep existing auth_state so signal keys persist.
    const credsFile = path.join(authDir, 'creds.json');
    const hasExistingCreds = fs.existsSync(credsFile);

    if (!hasExistingCreds) {
      logger.info('🆕 No existing auth state found — importing from SESSION_ID...');
...
async function handleConnectionUpdate(update) {
  try {
    ...
    if (connection === 'close') {
      ...
      if (isConflict) {
        logger.error('⛔ Session conflict: this WhatsApp account is already connected on another device/session. Close the other session and restart the bot with a fresh SESSION_ID.');
        process.exit(1);
    ...
    if (connection === 'open') {
      reconnectAttempts = 0; // Reset counter only on successful connection
      const jid = sock.user?.id || 'unknown';
      logger.info(`✅ Connected successfully! JID: ${jid}`);

      // Send a connected notification to the bot's own number (or owner if set)
      try {
