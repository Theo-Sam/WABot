...
async function startBot() {
  try {
    ...
    // NEW FIX: Actually call the import function if no existing creds but SESSION_ID is present
    if (!hasExistingCreds && config.SESSION_ID) {
      try {
        logger.info('📦 Found SESSION_ID in config. Importing to auth state...');
