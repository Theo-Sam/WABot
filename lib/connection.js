...
function cacheStatusMessage(msg, unwrappedStatusMessage, msgType) {
  ...
  const targetId = existingId || cacheId;
...
async function startConnection() {
  ...
  console.log(`[DESAM-STATUS] Auto status view is ${config.AUTO_STATUS_VIEW === "on" ? "ENABLED" : "DISABLED"}`);
  activeSock = sock;
  ...
  sock.ev.on("messaging-history.set", ({ messages: history }) => {
    ...
    for (const msg of history) {
      ...
      const msgType = getContentType(unwrappedStatusMessage);
      ...
      try {
        ...
        if (isStatusLikeJid(remoteJid) || isStatusLikeJid(msg.key?.participant || "")) {
          ...
          if (config.AUTO_STATUS_VIEW === "on") {
            sock.readMessages([msg.key]).then(() => {
              console.log(`[DESAM-STATUS] Marked status as viewed: ${msg.key?.id || "no-id"}`);
            }).catch((err) => {
              console.error(`[DESAM-STATUS] Failed to mark status viewed: ${msg.key?.id || "no-id"} | ${err.message}`);
            });
          } else {
