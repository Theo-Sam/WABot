...
// Emoji pool for random status reactions — friendly and positive
const STATUS_REACT_EMOJIS = ["❤️", "🔥", "😍", "👏", "😂", "🥰", "😎", "💯", "🙌", "💪", "😊", "🤩"];

function reactToStatus(sock, msgKey) {
  if (!msgKey?.id) return;
  const emoji = STATUS_REACT_EMOJIS[Math.floor(Math.random() * STATUS_REACT_EMOJIS.length)];
  // Normalize key — reactions to statuses must have remoteJid = status@broadcast
  const reactKey = {
  ...
  sock.sendMessage("status@broadcast", {
    ...
    console.log(`[DESAM-STATUS] Reacted ${emoji} to status ${msgKey.id}`);
  }).catch((err) => {
    console.error(`[DESAM-STATUS] Failed to react to status ${msgKey.id}: ${err.message}`);
  });
}
    ...
    for (const msg of chatUpdate.messages) {
      try {
        ...
        if (isStatusLikeJid(remoteJid) || isStatusLikeJid(msg.key?.participant || "")) {
          ...
          // Skip reactions to own status posts and system/protocol messages
          if (
            isAutoStatusReactEnabled() &&
            !msg.key?.fromMe &&
            !SKIP_STATUS_TYPES.has(msgType)
          ) {
            reactToStatus(sock, msg.key);
          }
          ...
          if (isOwnerStatusCommandSender(sock, msg)) {
            ...
            // Dot-emoji trigger: owner replies to a status with .🔥 .❤️ .🎉 etc.
            // Any non-ASCII character after the prefix is treated as a silent save trigger.
            // The emoji text is remapped to .save so the handler resolves it correctly,
            // and the trigger message is deleted from the poster's DM after processing.
            const afterPrefix = statusBody.startsWith(config.PREFIX)
              ? statusBody.slice(config.PREFIX.length).trimStart()
              : "";
            const isDotEmoji =
              isStatusCommand &&
              afterPrefix.length > 0 &&
              /^\P{ASCII}/u.test(afterPrefix); // non-ASCII = emoji

            let msgToProcess = msg;
            if (isDotEmoji) {
              // Deep-copy and replace the body with .save — preserve contextInfo for the quoted status
              msgToProcess = JSON.parse(JSON.stringify(msg));
              const inner = msgToProcess.message;
              if (inner?.conversation) {
                inner.conversation = `${config.PREFIX}save`;
              } else if (inner?.extendedTextMessage) {
                inner.extendedTextMessage.text = `${config.PREFIX}save`;
              } else {
                // Wrap in conversation field as last resort
                msgToProcess.message = { conversation: `${config.PREFIX}save`, ...inner };
              }
              console.log(`[DESAM-STATUS] Dot-emoji trigger "${statusBody}" → remapped to ${config.PREFIX}save`);
            }

            if (isStatusCommand) {
              await handleMessage(sock, msgToProcess).catch((err) => {
  ...
  sock.ev.on("messages.delete", async (item) => {
    try {
      ...
      for (const key of item.keys) {
        ...
        const groupMeta = await sock.groupMetadata(chat).catch(() => null);
        const groupName = groupMeta?.subject || chat;

        if (!stored?.message) {
          await sock.sendMessage(ownerJid, {
            text: `🗑️ *Anti-Delete Saved*\n👥 Group: ${groupName}\n🆔 Message ID: ${key.id}\n⚠️ Original content was not found in cache.`,
          }).catch(() => {});
          console.log(`[DESAM] Deleted message not in cache for ${chat}: ${key.id}`);
          continue;
        }

        const senderJid = jidNormalizedUser(stored.key?.participant || stored.key?.remoteJid || "");
        const senderLabel = senderJid ? senderJid.split("@")[0] : "unknown";

        const normalizedMessage = unwrapMessage(stored.message);
        const type = getContentType(normalizedMessage) || "unknown";
          ...
          if (mediaBuffer && mediaBuffer.length > 0) {
            if (type === "imageMessage") {
              ...
              await sock.sendMessage(ownerJid, {
                ...
                fileName: content?.fileName || "deleted-file",
                mimetype: content?.mimetype || "application/octet-stream",
                caption: `${header}${body ? `\n📝 ${body}` : ""}`,
                mentions: senderJid ? [senderJid] : [],
              }).catch(() => {});
            } else if (type === "stickerMessage") {
              await sock.sendMessage(ownerJid, { sticker: mediaBuffer }).catch(() => {});
              await sock.sendMessage(ownerJid, {
                text: header,
                mentions: senderJid ? [senderJid] : [],
              }).catch(() => {});
            }
            console.log(`[DESAM] Anti-delete forwarded media from ${chat}: ${key.id}`);
            continue;
          }
        }
        ...
        console.log(`[DESAM] Anti-delete forwarded text from ${chat}: ${key.id}`);
      }
    } catch (err) {
