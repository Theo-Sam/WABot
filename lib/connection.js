...
function extractStatusBody(unwrappedStatusMessage, msgType) {
  if (!unwrappedStatusMessage || !msgType) return "";
  const content = unwrappedStatusMessage[msgType] || {};
  if (msgType === "conversation") return String(content || "");
...
function cacheStatusMessage(msg, unwrappedStatusMessage, msgType) {
  ...
  const cacheId =
    msg.key.id ||
    `${msg.key.remoteJid || "unknown"}:${msg.key.participant || "unknown"}:${msg.messageTimestamp || Date.now()}:${statusCache.size}`;
