...
const { getGroupSettings, saveMessageToStore, getMessageFromStore, purgeOldMessages } = require("./database");
...
// SQLite-backed message store — persists across restarts, instant writes
function createBasicStore() {
  const chats = new Map();
  const contacts = new Map();

  // Purge old messages on startup and every 6 hours
  try { purgeOldMessages(); } catch {}
  setInterval(() => { try { purgeOldMessages(); } catch {} }, 6 * 60 * 60 * 1000);

  const store = {
    bind(ev) {
      // Persist every incoming message immediately to SQLite
      ev.on("messages.upsert", ({ messages: msgs }) => {
        for (const msg of msgs || []) {
          if (msg?.key?.id) {
            try { saveMessageToStore(msg); } catch {}
          }
        }
      });
      ...
      // Chat events (kept in-memory only — not needed for anti-delete)
      ev.on("chats.upsert", ({ chats: newChats }) => {
        for (const chat of newChats || []) {
          if (chat?.id) chats.set(chat.id, chat);
        }
      });
      // Contact events (kept in-memory only)
    ...
    async getMessage(key) {
      try { return getMessageFromStore(key.id); } catch { return null; }
    },
...
function buildStatusReadKey(key = {}) {
  const participantCandidate =
    ...
    key.participant ||
    "";
  const normalizedParticipant = participantCandidate ? jidNormalizedUser(participantCandidate) : undefined;
...
function markStatusViewed(sock, key, sourceLabel = "upsert") {
  ...
  if (!normalizedKey.participant) {
    console.log(`[DESAM-STATUS] Skipping status view mark (${sourceLabel}): missing participant for ${normalizedKey.id}`);
    return;
  }
